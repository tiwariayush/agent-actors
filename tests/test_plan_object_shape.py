...
class NormalizePlanTasksTests(unittest.TestCase):
    ...
    def test_single_task_object_wrapped_as_list(self):
        """LLMs often emit one object instead of a one-element array."""
        task = {
            "child_id": 0,
            "task_id": 0,
            "task": "Research AGI",
        ...
        self.assertEqual(normalize_plan_tasks(task), [task])

    def test_non_collection_plan_rejected(self):
        with self.assertRaises(TypeError) as ctx:
            normalize_plan_tasks("not json structure")
        self.assertIn("JSON array or task object", str(ctx.exception))


class PlanObjectShapeParentTests(unittest.TestCase):
    ...
    def test_single_task_object_dispatches_instead_of_typeerror(self):
        """Before the fix, iterating a task dict yielded string keys and crashed."""
        remote_calls = []
