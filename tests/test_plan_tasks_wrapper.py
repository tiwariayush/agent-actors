...
class NormalizePlanTasksWrapperTests(unittest.TestCase):
    ...
    def test_tasks_wrapper_unwrapped(self):
        """LLMs commonly wrap the required array as {"tasks": [...]}."""
        task = {
            "child_id": 0,
            "task_id": 0,
            "task": "Research AGI",
        ...
        self.assertEqual(normalize_plan_tasks({"tasks": [task]}), [task])

    def test_subtasks_wrapper_unwrapped(self):
        task = {
            "child_id": 0,
            "task_id": 0,
            "task": "Write summary",
        ...
        self.assertEqual(normalize_plan_tasks({"subtasks": [task]}), [task])

    def test_single_task_under_tasks_key_wrapped(self):
        task = {
            "child_id": 0,
            "task_id": 0,
            "task": "Research AGI",
    ...
    def test_non_collection_plan_rejected(self):
        with self.assertRaises(TypeError) as ctx:
            normalize_plan_tasks("not json structure")
        self.assertIn("JSON array, tasks object, or task object", str(ctx.exception))


class PlanTasksWrapperParentTests(unittest.TestCase):
    ...
    def test_tasks_wrapper_dispatches_instead_of_typeerror(self):
        """Before the fix, iterating {"tasks": [...]} yielded 'tasks' and crashed."""
        remote_calls = []
